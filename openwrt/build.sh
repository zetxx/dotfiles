...
echo "--------------------------------------------------------------------"
echo "Packages: ${PREINSTALLED_PACKAGES}"
echo "--------------------------------------------------------------------"

make image PROFILE=${TARGET_PLATFORM} PACKAGES="${PREINSTALLED_PACKAGES} -dnsmasq -kmod-ipv6 -kmod-nf-ipt6 -kmod-ip6tables -odhcp6c -kmod-ip6tables -wpad-mini -ppp-mod-pppoe -kmod-ppp -kmod-pppox -kmod-pppoe" FILES=${IMGTEMPDIR}
